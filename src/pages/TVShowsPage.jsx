  ...
  const fetchTVShows = async (pageNum = 1) => {
    ...
    try {
      ...
      if (data.results && data.results.length > 0) {
        ...
        setShows((prev) => {
          const newShows = normalizedResults.filter(
            (item) => !prev.some((s) => s.id === item.id)
          );
        ...
        setHasMore(false);
        if (shows.length === 0) setError("No trending TV shows found.");
      }
    } catch (err) {
    ...
    <div className="min-h-screen bg-gray-950 text-white flex flex-col">
      ...
      <section className="px-[60px] flex flex-col gap-6">
        ...
        {loading && shows.length === 0 && (
          <p className="text-center mt-8">Loading TV shows...</p>
        )}
        ...
        >
          {shows.map((show) => (
            <div key={`${show.id}-tv`} className="flex justify-center">
        ...
        {loading && shows.length > 0 && (
          <p className="text-center mt-8 text-gray-400">Loading more TV shows...</p>
        )}
      </section>
    </div>
  );
